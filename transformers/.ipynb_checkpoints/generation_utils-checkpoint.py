...
class GenerationMixin:
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        # use_student = model_kwargs.get('use_student', True)
        # # AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
        ...
        while True:
            ...
            # AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
                ...
                # AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
                # next_token_logits_student = outputs_student.logits[:, -1, :]
                hidden_state = outputs.decoder_hidden_states[1][:, -1, :] # Use hidden layer 1
                next_token_logits_student = self.lm_head(hidden_state)
                # print("We change here!!!")
                # AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
                next_token_logits_student = nn.functional.log_softmax(
                    next_token_logits_student, dim=-1
                ) 
            ...
            # # AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
            # else:
            #     # Placeholder logits if student is unused — same shape as needed
            #     next_token_logits_student = input_ids.new_zeros(
            #         input_ids.size(0), self.config.vocab_size, dtype=torch.float
            #     )
            # # AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
            ...
            # # Early exit based on total variation distance (TV) between teacher and student
            ...
            # teacher_probs = next_token_scores.exp()
            # student_probs = next_token_logits_student.exp()
            # tv_dist = torch.abs(teacher_probs - student_probs).sum(dim=-1)  # shape: (batch_size * num_beams)
            # # print(tv_dist)
            
            # # If all beams are very close (agree well), we stop early
            # early_exit_threshold = 1  # fixed threshold
            # use_student = True
            # if torch.all(tv_dist < early_exit_threshold):
            #     # print('Yo')
            #     use_student = False
            
            # if not use_student:
            #     next_token_logits_student = torch.zeros_like(next_token_logits)  # dummy to avoid errors
            # # AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
            ...
            if model_kwargs['teacher_student']:
                ...
                model_kwargs_student = self._update_model_kwargs_for_generation(
                    outputs, model_kwargs_student, is_encoder_decoder=self.config.is_encoder_decoder
                )
                # AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
